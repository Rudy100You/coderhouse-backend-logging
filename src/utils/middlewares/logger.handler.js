    ...
    transports: [
        new winston.transports.Console({
            ...
            format: winston.format.combine(
                ...
                winston.format.printf(logData=> `${logData.timestamp} [${logData.level}]: ${logData.message} ${logData.splat!==undefined?`${logData.splat}`:" "}`)
            )
        }),
...
export const addLogger = (req,res,next) => {
    req.logger = logger;
    req.logger.info(`${req.method} en ${req.url} - ${new Date().toLocaleTimeString().timestamp}` )
    next()
}